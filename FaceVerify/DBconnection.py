...
def login(email,password):
    ...
    cursor = db.cursor()
    cemail = Cipher.encrypt(email)[0:]
    cpassword = Cipher.encrypt(password)[0:]
    print (cemail)
    print(cpassword)
    sql = "SELECT * FROM musuarios where email_usr={} ".format(cemail)
    sql = sql + "&& pass_usr={} limit 1".format(cpassword)
    print(sql)
    cursor.execute(sql)
